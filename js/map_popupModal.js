$(document).ready(function () {
  require([
    ...
    $("#listModal").on("click", ".listItem", function () {
      ...
      console.log("clicked item from list", clickItemID)
      var testQuery = projectLocations.createQuery();
      testQuery.returnGeometry = true;
      testQuery.outFields = ["*"];

      testQuery.where = "ProjectID = '" + clickItemID + "'";
      projectLocations.queryFeatures(testQuery).then(function (ids) {
        testClick.geometry = ids.features[0].geometry;
        view.graphics.add(testClick)
        view.goTo(testClick)
      });
      view.popup.open({
        location: testClick,
